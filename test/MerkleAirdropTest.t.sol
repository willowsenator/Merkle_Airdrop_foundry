...
contract MerkleAirdropTest is Test {
    ...
    function setUp() public {
        ...
        merkleRoot = keccak256(abi.encode("test_merkle_root"));
    ...
    function testPreventDoubleClaim() public {
        ...
        // First claim should work (with proper merkle proof)
        // Second claim should fail
        
        // For this test, let's check the hasClaimed mapping directly
        assertFalse(airdrop.hasClaimed(user1), "User1 should not have claimed initially");
        
        // Test double claim prevention by checking the error
        vm.expectRevert(MerkleAirdrop.MerkleAirdrop__AlreadyClaimed.selector);
        
        // Manually set the user as having claimed to test the revert
        vm.store(
            address(airdrop),
            keccak256(abi.encode(user1, uint256(2))), // slot for s_hasClaimed[user1]
            bytes32(uint256(1))
        );
